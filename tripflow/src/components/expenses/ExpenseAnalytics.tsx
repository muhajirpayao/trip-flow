...
interface Props { expenses: Expense[]; currency: Currency }
...
export default function ExpenseAnalytics({ expenses, currency }: Props) {
  ...
  const byDay = useMemo(() => {
    const map: Record<string, number> = {};
    expenses.forEach(e => { map[e.expenseDate] = (map[e.expenseDate] ?? 0) + e.amount; });
    return Object.entries(map)
  ...
  return (
    ...
    >
      ...
      <div className="glass rounded-2xl border border-slate-100 p-4 shadow-sm">
        <p className="mb-3 text-xs font-bold uppercase tracking-wide text-slate-500">Spending by Category</p>
        <div className="flex items-center gap-4">
          <div className="h-36 w-36 shrink-0">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={byCategory} cx="50%" cy="50%" innerRadius={36} outerRadius={60}
                  dataKey="value" strokeWidth={2} stroke="#fff">
                  {byCategory.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                </Pie>
                <Tooltip
                  formatter={(value: any) => fmtCurrency(Number(value ?? 0), currency)}
                  contentStyle={{ borderRadius: 12, border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', fontSize: 11 }}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
            ...
            {byCategory.slice(0, 6).map(item => {
              ...
              return (
                <div key={item.name} className="flex items-center gap-2">
                  ...
                  <span className="min-w-0 flex-1 truncate text-[11px] text-slate-600">{item.name}</span>
                  <span className="text-[11px] font-bold text-slate-700">{pct}%</span>
                  <span className="text-[10px] text-slate-400">{fmtCurrency(item.value, currency)}</span>
                </div>
              );
            })}
          </div>
      ...
      {byDay.length > 1 && (
        <div className="glass rounded-2xl border border-slate-100 p-4 shadow-sm">
          <p className="mb-3 text-xs font-bold uppercase tracking-wide text-slate-500">Daily Spending</p>
          <div className="h-44">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={byDay} margin={{ top: 0, right: 0, left: -10, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ borderRadius: 12, border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', fontSize: 11 }}
                />
                <Bar dataKey="total" radius={[6, 6, 0, 0]} fill="url(#barGrad)" maxBarSize={32} />
                <defs>
