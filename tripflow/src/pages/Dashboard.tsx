...
import { Calendar, Wallet, MapPin, CheckSquare, Plus, ArrowRight, RefreshCw } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const resetTrip = () => {
    if (confirm('Clear current trip and start over?')) {
      clearTrip();
      navigate('/');
    }
  };
  ...
  return (
    <div>
      {/* Header */}
      <div className="gradient-primary pt-12 pb-16 px-5 relative overflow-hidden">
        ...
        <div className="relative z-10 flex justify-between items-start">
          <div>
            ...
            <h1 className="text-white text-2xl font-black leading-tight mb-1">{trip.destination}</h1>
            <p className="text-indigo-200 text-sm">
              {days === 0 ? 'Your adventure starts today! 🎉' : `${days} day${days === 1 ? '' : 's'} to departure`}
            </p>
          </div>
          <button onClick={resetTrip}
      ...
      <div className="px-4 -mt-10 pb-6 relative z-10 space-y-4">
        {/* Countdown card */}
        <div className="bg-white rounded-3xl p-5 shadow-card flex items-center gap-4">
          <div className="gradient-sky w-16 h-16 rounded-2xl flex flex-col items-center justify-center text-white flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-500">{total} days · {trip.travelType.charAt(0).toUpperCase() + trip.travelType.slice(1)} trip {typeIcon}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={s.label} className="bg-white rounded-2xl p-4 shadow-card">
              ...
              <p className={`font-black text-slate-900 leading-tight ${s.small ? 'text-base' : 'text-2xl'}`}>{s.value}</p>
        ...
        <div className="bg-white rounded-3xl p-5 shadow-card">
          ...
          {[
            ...
            <div key={label} className={`flex justify-between items-center py-3 ${i < arr.length - 1 ? 'border-b border-slate-50' : ''}`}>
