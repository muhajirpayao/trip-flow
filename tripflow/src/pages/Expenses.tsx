...
export default function Expenses() {
  ...
  const handleSubmit = async (form: ExpenseFormData) => {
    ...
    if (editing) {
      ...
      setExpenses(prev =>
        prev.map(e => (e.id === editing.id ? optimistic : e)),
      );
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!ok) load(); // rollback on failure
  };
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-slate-50 via-violet-50/30 to-pink-50/20">
      ...
      <motion.button
        whileTap={{ scale: 0.93 }}
        onClick={openAdd}
        className="fixed bottom-20 right-4 z-30 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500 to-purple-600 shadow-xl shadow-violet-300 transition-opacity hover:opacity-90"
      >
