...
import { Users, Plane, FileText, Bell, MapPin, DollarSign, TrendingUp } from 'lucide-react';
import {
...
const FEATURE_USAGE = [
  { name: 'Itinerary',      value: 45, color: '#7c3aed' },
  { name: 'Documents',      value: 20, color: '#ec4899' },
  { name: 'Expenses',       value: 15, color: '#0ea5e9' },
  { name: 'Places',         value: 10, color: '#10b981' },
...
export default function AdminDashboard() {
  ...
  const s = stats ?? { totalUsers: 0, totalTrips: 0, totalDocuments: 0, totalNotifications: 0, savedPlaces: 0, totalExpenses: 0 };
  ...
  return (
    <div className="space-y-6 max-w-7xl">
            ...
            <BarChart data={DAILY_ACTIVE} barSize={20}>
              ...
              <Bar dataKey="users" radius={[6, 6, 0, 0]}>
                {DAILY_ACTIVE.map((_, i) => (
                  <Cell key={i} fill={i === 4 ? '#7c3aed' : '#ddd6fe'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      {/* Feature usage + platform health */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <ChartCard title="Feature Usage">
          <ResponsiveContainer width="100%" height={180}>
            <PieChart>
              <Pie data={FEATURE_USAGE} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={3} dataKey="value">
          ...
          <div className="grid grid-cols-2 gap-1 mt-2">
            {FEATURE_USAGE.map((f) => (
              <div key={f.name} className="flex items-center gap-1.5 text-xs text-gray-500">
        ...
        <div className="lg:col-span-2 bg-white border border-violet-100 rounded-2xl p-5 shadow-sm">
          ...
          <div className="space-y-3">
            {[
              { label: 'User Activation Rate',   value: 78, color: '#7c3aed' },
              { label: 'Trip Completion Rate',    value: 62, color: '#ec4899' },
              { label: 'Document Upload Rate',    value: 45, color: '#0ea5e9' },
              { label: 'Expense Tracking Rate',   value: 33, color: '#10b981' },
            ].map((item) => (
              <div key={item.label}>
